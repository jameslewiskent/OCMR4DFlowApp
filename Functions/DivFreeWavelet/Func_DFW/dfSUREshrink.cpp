...
#define PERCENTZERO_IN prhs[6]
#define SIGMA_IN prhs[7]
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  if (nrhs !=8) {
    mexErrMsgTxt("dfSUREshrink requires 8 input arguments.");
  }
  ...
  dims_mx = (mwSize*) malloc(sizeof(int)*3);
  int i;
  for (i = 0; i<3; i++)
    {
      minSize[i] = (int) (minSizeDouble[i]);
      dims[i] = (int) fovDouble[i];
      dims_mx[i] = (mwSize) fovDouble[i];
    }
  struct dfwavelet_plan_s* plan = prepare_dfwavelet_plan(3,(int*)dims,minSize,res,0);

  plan->percentZero = percentZero;
  
  dims_mx[0] = 1;
  dims_mx[1] = plan->numCoeff;
  
  sigma = mxGetPr(SIGMA_IN)[0];
  
  // Create a matrix for the return argument 
  WCDF1_OUT = mxCreateNumericArray(2, dims_mx, mxDOUBLE_CLASS, mxREAL);
  WCDF2_OUT = mxCreateNumericArray(2, dims_mx, mxDOUBLE_CLASS, mxREAL);
  WCN_OUT = mxCreateNumericArray(2, dims_mx, mxDOUBLE_CLASS, mxREAL);
  ...
  dfSUREshrink_cpu(plan,sigma,out_wcdf1,out_wcdf2,out_wcn);

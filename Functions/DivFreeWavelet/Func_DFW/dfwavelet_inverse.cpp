...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  int *dims,*minSize;
  mwSize *dims_mx; // ATH added, to replace dims for use as size_t variable
  double* numlevels;
  double* wcSizes;
  ...
  dims_mx = (mwSize*)malloc(sizeof(mwSize)*3);
  int i;
  for (i = 0; i<3; i++)
    {
  ...
  struct dfwavelet_plan_s* plan = prepare_dfwavelet_plan(3,dims,minSize,res,0);  // this crashes matlab...

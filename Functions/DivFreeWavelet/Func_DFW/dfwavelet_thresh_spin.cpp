...
#define DFTHRESH_IN prhs[5]
#define NTHRESH_IN prhs[6]
#define SPINS_IN prhs[7]
#define ISRAND_IN prhs[8]
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  if (nrhs !=9) {
    mexErrMsgTxt("dfwavelet_thresh requires 9 input arguments.");
  }
  ...
  for (i = 0; i<3; i++)
    {
      ...
      dims[i] = (int) (dims_mx[i]);
    }
  
  // Create a matrix for the return argument 
  VX_OUT = mxCreateNumericArray(3, dims_mx, mxDOUBLE_CLASS, mxREAL);
  VY_OUT = mxCreateNumericArray(3, dims_mx, mxDOUBLE_CLASS, mxREAL);
  VZ_OUT = mxCreateNumericArray(3, dims_mx, mxDOUBLE_CLASS, mxREAL);
  ...
  dfwavelet_thresh_spin(plan,dfthresh,nthresh,spins,isRand,out_vx,out_vy,out_vz,in_vx,in_vy,in_vz);

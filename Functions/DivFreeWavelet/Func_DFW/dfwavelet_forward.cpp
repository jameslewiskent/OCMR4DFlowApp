...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  dims_mx =  (mwSize*)mxGetDimensions(VX_IN);
  dims_i = (int*) malloc(sizeof(int)*3);
  for (i = 0; i<3; i++)
    {
      minSize[i] = (int) (minSizeDouble[i]);
      dims_i[i] = (int)dims_mx[i];
    }
  
  plan = prepare_dfwavelet_plan(3,dims_i,minSize,res,0);
...
// Create a matrix for the return argument 
  ...
  NUMLEVELS = mxCreateNumericArray(1,dims_mx,mxDOUBLE_CLASS,mxREAL);
  dims_mx[1] = (plan->numLevels+2)*3;
  WC_SIZES = mxCreateNumericArray(2,dims_mx,mxDOUBLE_CLASS,mxREAL);

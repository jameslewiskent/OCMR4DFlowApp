...
% JK renamed these in case of lots of series indices, leading to ridiculously long filenames
if length(ser_flow) > 4
    series_flow_hash = hash_series_flow(ser_flow);
    mat_file_raw = fullfile(d_path,['flow_raw_',series_flow_hash,'.mat']);
    mat_file_pro = fullfile(d_path,['flow_pro_',series_flow_hash,'.mat']);
else
...
clear im3D % JK clear to save memory
...
if((sum(abs(NormSCT)>0)>1)||(sum(abs(info.ImageOrientationPatient(1:3))>0)>1))
    app.DialogBoxTextArea.Value{length(app.DialogBoxTextArea.Value)+1} = char(string(datetime('now','Format','HH:mm')) + ' - WARNING! Image appears rotated, this may not be supported. '); pause(0.1); scroll(app.DialogBoxTextArea, 'bottom');
end
...
for iC = 1:length(info.order.contrastsList)
    switch(info.order.contrastsList{iC}(end-1:end))
        case 'in' % note this is the slice direction
            ...
            if(orienation == 1)  % in prisma data it appears that sagital has a sign reversal for sagital data, must check more
                flow_sign(3) = -1;
            end
        case 'fh'
...
for i = 1:3
    ...
    flow(:,:,:,:,i) = sum(bsxfun(@times,flow_in,v),flow_dim);
end
...
% 8. Save results
save(mat_file_pro,'flow','flow_mag','stasis','peak_flow','CRS2SCTflow','info','mag','pcmri','flow_unmasked','vorticity','vorticity_map','cav','lambda2','-v7.3');
out_data.flow = flow;

function [contours, study_uid, series_id, image_uid] = ExtractCMR42Contours(fname)
% Aaron Hess
...
%
% JK updated Feb 2024 to also find series UID form countours
%
...
for i = 0:elist.size()-1
    ...
    % JK added for finding SeriesUID
    if strcmp(elist.get(i).getAttribute(0).getValue , 'ModuleStates')
        ModuleStates = elist.get(i);
    end
end

% JK added below for finding SeriesUID
elist2 = ModuleStates.getChildElements();
for i = 0:elist2.size()-1
    if strcmp(elist2.get(i).getAttribute(0).getValue , 'ModuleSAX3DFunction')
        ModuleSAX3DFunction = elist2.get(i);
    end
end
elist2 = ModuleSAX3DFunction.getChildElements();
for i = 0:elist2.size()-1
    if strcmp(elist2.get(i).getAttribute(0).getValue , 'SeriesSelection')
        SeriesSelection = elist2.get(i);
    end
end
elist2 = SeriesSelection.getChildElements();
for i = 0:elist2.size()-1
    if strcmp(elist2.get(i).getAttribute(0).getValue , 'SAX')
        SAX = elist2.get(i);
    end
end
elist2 = SAX.getChildElements();
for i = 0:elist2.size()-1
    if strcmp(elist2.get(i).getAttribute(0).getValue , 'Series')
        Series = elist2.get(i);
    end
    if strcmp(elist2.get(i).getAttribute(0).getValue , 'ImageUid')
        ImageUid = elist2.get(i);
    end
end
series_id = str2double(Series.getValue().toCharArray()');
image_uid = ImageUid.getValue().toCharArray()';
...
% JK clear variables from workspace otherwise java isn't cleared
clearvars -except contours study_uid series_id image_uid
end
